...
import { tokenBlacklist } from "../config/TokenBlacklist.js"; // <--- IMPORT NECESSÁRIO

const router = express.Router();

// Validação padrão para criação/login de usuário.
const validateUser = [
  body("username")
    .trim()
...
// POST /registerRegistra novo usuário.
...
// POST /logout Adiciona token na blacklist (invalida até expirar).
router.post("/logout", authenticateToken, (req, res) => {
  const token = req.headers.authorization?.split(" ")[1];

  // Se o token existir, adiciona à blacklist
  if (token) {
